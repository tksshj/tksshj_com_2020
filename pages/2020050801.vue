<template>
  <div>
    ...
    <tc-page-content :playButton="true">
      <p>
        なんでもいいよって言われると困るみたいなのあるじゃないですか。
        何か制限があったほうが行動しやすいとか、思考しやすいとか。
        そういう制限とかって実はやる気のタネみたいなものなのかもしれないですね。
      </p>
      <p>
        でもあれか、締切明日っていうの結構な制限かと思うんですけど、やる気出てこないですね。
        制限の種類が違うのかな。
      </p>
    </tc-page-content>
    ...
    draw() {
      ...
      for (let row = 0; row < this.nVerticalTiles; row++) {
        for (let col = 0; col < this.nHorizontalTiles; col++) {
          ...
          this.p5.rotate(Math.PI * progress * (col % 2 == 0 ? 1.0 : -1.0))
